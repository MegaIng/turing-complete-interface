...
proc state_to_binary*(save_version: int, components: seq[parse_component], wires: seq[parse_wire], nand: uint32, delay: uint32, menu_visible: bool, clock_speed: uint32, nesting_level: uint8, description: string, camera_position: point): seq[uint8]  {. exportpy .} =
