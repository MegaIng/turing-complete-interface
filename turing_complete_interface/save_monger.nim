...
type py_parse_component* = object
  kind*: string
  position*: point
  rotation*: uint8
  permanent_id*: int
...
proc py_parse_state*(input: string): (seq[py_parse_component], seq[py_parse_circuit], uint32, uint32, int) {.exportpy.} =
  let (components, circuits, nand, delay, level_version) = parse_state(input)
  (
    components.map((pc) => py_parse_component(kind: $pc.kind, position: pc.position, rotation: pc.rotation, permanent_id: pc.permanent_id, custom_string: pc.custom_string)),
    circuits.map((pc) => py_parse_circuit(permanent_id: pc.permanent_id, path: pc.path, kind: $pc.kind, color: pc.color, comment: pc.comment)),
    nand, delay, level_version
  )

proc parse_state_to_string*(parse_components: seq[py_parse_component], parse_circuits: seq[py_parse_circuit], nand: uint32, delay: uint32, level_version: int): string {.exportpy.} =
  parse_state_to_string(parse_components.map((ppc) => parse_component(kind:parse_enum[component_kind](ppc.kind), position: ppc.position, rotation: ppc.rotation, permanent_id: ppc.permanent_id, custom_string: ppc.custom_string)),
                        parse_circuits.map((ppc) => parse_circuit(permanent_id: ppc.permanent_id, path: ppc.path, kind:parse_enum[circuit_kind](ppc.kind), color: ppc.color, comment: ppc.comment)),
                        nand, delay, level_version)

proc is_virtual*(component: string): bool {. exportpy .} =
  parse_enum[component_kind](component) in virtual_kinds